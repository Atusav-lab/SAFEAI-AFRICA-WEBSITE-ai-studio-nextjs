...
const products = [
  {
    ...
    desc: 'SAFESeq is Africa\'s first unified, AI-powered genomic analysis platform — built to close the continent\'s genomic data gap. Instead of juggling five or more disconnected foreign tools to analyse a single DNA sequence, researchers, students, and public health institutions can now run CRISPR guide design, primer design, plasmid mapping, 3D protein structure prediction, antimicrobial resistance (AMR) screening, and phylogenetic analysis — all from one browser tab, with no installation and no code required. Every result is interpreted in plain language by a built-in AI assistant trained on Uganda\'s local pathogen landscape, and the platform is the first in Africa to expose its tools via Model Context Protocol (MCP), making it natively usable by AI agents like Claude and ChatGPT. Data stays on Uganda-controlled cloud infrastructure — sovereign by design.',
